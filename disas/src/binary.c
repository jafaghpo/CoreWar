...
/*   Updated: 2017/12/10 18:37:54 by jafaghpo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_uint8	*file_to_string(char *path, int *ret)
{
	...
	if ((fd = open(path, O_RDONLY)) < 0)
		return (!print_error(errno, __func__, __FILE__, __LINE__) ? NULL : str);
	if ((*ret = read(fd, tmp, BSIZE)) < 0)
		return (!print_error(errno, __func__, __FILE__, __LINE__) ? NULL : str);
	if (*ret == BSIZE)
		return (!print_error(EFBIG, __func__, __FILE__, __LINE__) ? NULL : str);
...
int				get_binary(t_file *file, char *path)
{
	...
	prog_size = ret - HEADER_SIZE;
	if (!(file->prog = malloc(sizeof(char) * prog_size)))
		return (print_error(errno, __func__, __FILE__, __LINE__));
	ft_memcpy(file->prog, str + HEADER_SIZE, prog_size);
	return (1);
}
