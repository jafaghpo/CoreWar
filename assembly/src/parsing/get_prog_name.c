...
/*   Updated: 2018/03/03 20:55:54 by jafaghpo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_name(char *line, t_tab *current, int *end)
{
	int		len;
...
int			get_prog_name(t_tab *tab, t_file *file, t_visual *win)
{
	...
	while (end != 1 && (ret = ft_getline(&line, file)))
	{
		...
		if (!end && (!*line || *line == COMMENT_CHAR))
			free(current.line);
		else if (parse_name(line, &current, &end))
